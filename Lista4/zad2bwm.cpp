...
for (int j = 1; j <= 10000; j++ ) {
	...
	for (int i = 0; i < 1; i++ ) {
		...
		randomPermutationArray( mainArray, j );

		for ( int k = 0; k < j; k++ ) {

			insert( newTree, (*mainArray)[k] );
		...
		for (int l = 0; l < 1000; l++) {
			...
			findForTest( newTree->root, std::rand() % j, &counterCompare );
			if ( l == 0) {
				compareCount->push_back (counterCompare);
			} else {
				(*compareCount)[j-1] = ( (*compareCount)[j-1] + counterCompare) / 2;
			}
		}
		delete newTree;
		delete mainArray;
